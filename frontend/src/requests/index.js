...
class AjaxRequests {
  ...
  getHomes = async () => {
    try {
      const { data } = await this.directusClient.getItems("homes", {
        fields: "*, translations.*, img.data.*",
      });
  ...
  getRealtors = async () => {
    const { data } = await this.directusClient.getItems("realtors", {
      fields: "*, img.data.*",
    });
    return data.map((realtor) => ({
      img:
        process.env.DIRECTUS_PUBLIC_URL +
        realtor.img.data.thumbnails[6].relative_url,
      alt: realtor.alt_img,
  ...
  getGalleryItems = async () => {
    const { data } = await this.directusClient.getItems("gallery_items", {
      fields: "*, img.data.*",
    });
    return data.map((galleryItem) => ({
      img:
        process.env.DIRECTUS_PUBLIC_URL +
        galleryItem.img.data.thumbnails[5].relative_url,
    }));
  };
  ...
  getHeaderMetaData = async () => {
    const { data } = await this.directusClient.getItems("header_metadata", {
      fields:
        "*, logo_img.data.*, translations.*, seen_on_logos.img.data.*, seen_on_logos.alt_img",
    });
    const raw = data[0];
    return {
      logoImg:
        process.env.DIRECTUS_PUBLIC_URL +
        raw.logo_img.data.thumbnails[6].relative_url,
      logoAlt: raw.logo_alt,
      seenOnLogos: raw.seen_on_logos.map((seenOnLogo) => ({
        img:
          process.env.DIRECTUS_PUBLIC_URL +
          seenOnLogo.img.data.thumbnails[6].relative_url,
  ...
  getRealtorsMetaData = async () => {
    const { data } = await this.directusClient.getItems("realtors_metadata", {
      fields: "*, translations.*",
    });
    const raw = data[0];
    return {
  ...
  getHomesMetaData = async () => {
    const { data } = await this.directusClient.getItems("homes_metadata", {
      fields: "*, translations.*",
    });
    const raw = data[0];
    return {
  ...
  getFeaturesMetaData = async () => {
    const { data } = await this.directusClient.getItems("features_metadata", {
  ...
  getStoriesMetaData = async () => {
    const { data } = await this.directusClient.getItems("stories_metadata", {
    ...
    return {
      ...
      img1:
        process.env.DIRECTUS_PUBLIC_URL +
        raw.img_1.data.thumbnails[5].relative_url,
      alt1: raw.alt_1,
      img2:
        process.env.DIRECTUS_PUBLIC_URL +
        raw.img_2.data.thumbnails[5].relative_url,
      alt2: raw.alt_2,
    };
  };
}
