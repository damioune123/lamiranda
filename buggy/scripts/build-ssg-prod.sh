...
docker exec -ti $(docker ps -aqf "name=lamiranda_express_prod") npm run build:prod
echo "Connecting to express_prod and exporting it with Next.js SSG"
docker exec -ti $(docker ps -aqf "name=lamiranda_express_prod") npm run export:prod
